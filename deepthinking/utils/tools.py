...
def get_model(model, width, max_iters, in_channels=3,testing=False):
    model = model.lower()
    net = getattr(models, model)(width=width, in_channels=in_channels, max_iters=max_iters,testing=testing)
    return net
...
def load_model_from_checkpoint(problem, model_args, device,testing=False):
    ...
    net = get_model(model, width, in_channels=in_channels, max_iters=max_iters,testing=testing)
    ...
    if model_path is not None:
        ...
        # Adjust for 'module.' prefix in state_dict keys if present
        if any(key.startswith('module.') for key in net.state_dict().keys()):
            state_dict["net"] = {f'module.{key}' if not key.startswith('module.') else key: value for key, value in state_dict["net"].items()}
        else:
            state_dict["net"] = {key[len('module.'):]: value for key, value in state_dict["net"].items() if key.startswith('module.')}

        if parallel==True:
            translation_map = deparallel_keys(state_dict["net"].keys())
            state_dict["net"] = {translation_map[key]: value for key, value in state_dict["net"].items()}

        # # Print state_dict keys and their shapes
        # print("Keys and shapes in state_dict['net']:")
        # for key, value in state_dict["net"].items():
        #     print(f"{key}: {value.shape}")

        # # Print net state_dict keys and their shapes
        # print("\nKeys and shapes in net.state_dict():")
...
def deparallel_keys(state_dict_keys):

    translation_map = {
        'module.recur_block.0.weight': 'module.conv_recall.weight',
        'module.head.4.weight': 'module.head_conv3.weight',
        'module.projection.0.weight': 'module.projection.0.weight',
        'module.recur_block.1.0.conv1.weight': 'module.recur_block_0.conv1.weight',
        'module.recur_block.1.0.conv2.weight': 'module.recur_block_0.conv2.weight',
        'module.recur_block.1.1.conv1.weight': 'module.recur_block_1.conv1.weight',
        'module.recur_block.1.1.conv2.weight': 'module.recur_block_1.conv2.weight',
        'module.head.0.weight': 'module.head1.0.weight',
        'module.head.2.weight': 'module.head2.0.weight'
    }

    translated_keys = {}
    for old_key in state_dict_keys:
        if old_key in translation_map:
            translated_keys[old_key] = translation_map[old_key]
        else:
            # If no translation is found, keep the key as is
            translated_keys[old_key] = old_key
