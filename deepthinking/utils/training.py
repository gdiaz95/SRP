...
def get_output_for_prog_loss(inputs, max_iters, net):
    ...
    outputs, _ = net(inputs, iters_elapsed=n, iters_to_do=k, interim_thought=interim_thought,testing=False)
    return outputs, k
...
def train_progressive(net, loaders, train_setup, device):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader, leave=False)):
        ...
        outputs_max_iters, _ = net.forward(inputs, iters_to_do=max_iters,testing=False)
        if alpha != 1:
...
############################################################################################
        ...
        #         if 'weight' in name:
        #             if j==0:
        #                 mask = (param.abs() < 1e-5).float()
        #                 mask_grad.append(mask)
        #             param.grad *= mask_grad[i]
############################################################
