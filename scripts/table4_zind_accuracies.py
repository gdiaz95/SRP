#!/usr/bin/env python3
"""table4_zind_accuracies.py

Recompute Table 4 ZInD floor-plan accuracies (Wavefront MST, Mehlhorn, Kou)
on the discretized floor-plan test sets (5_green, 6_green, 7_green).

For each terminal count:
  - Loads inputs.npy / solutions.npy from
      data/zind/big_floorplans/{n}_green/maze_data_test_21/
  - Parses the floor-plan image into a NetworkX graph
  - Runs Wavefront MST, Mehlhorn, and Kou on each maze
  - Computes accuracy = fraction of mazes where approx length <= optimal length

Results are saved to results/ratios_calculation_rooms/ and a Table 4 summary
is printed at the end.

Usage (run from project root):
    python scripts/table4_zind_accuracies.py
"""
...
RESULT_DIR = os.path.join("results", "ratios_calculation_rooms")
DATA_TYPES = ["5_green", "6_green", "7_green"]

os.makedirs(RESULT_DIR, exist_ok=True)


# ── Graph utilities (same as table2_approx_accuracies.py) ─────────────────────
...
def run_data_type(data_type):
    ...
    return acc_wave, acc_kou, acc_meh
...
def main():
    print("=" * 72)
    print("  Table 4: ZInD floor-plan accuracy — graph approximation algorithms")
    print("=" * 72)
    ...
    for data_type in DATA_TYPES:
        print(f"\n  [{data_type}]")
        t0 = time.time()
        acc_wave, acc_kou, acc_meh = run_data_type(data_type)
        elapsed = time.time() - t0
        results[data_type] = (acc_wave, acc_kou, acc_meh)
        print(f"    Wavefront MST : {acc_wave:.2f}%")
        print(f"    Kou           : {acc_kou:.2f}%")
        print(f"    Mehlhorn      : {acc_meh:.2f}%")
