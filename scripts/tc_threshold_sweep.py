...
Sweep the TC path-intensity threshold and report accuracy on the synthetic
5-terminal validation set and the 6- and 7-terminal test sets.

Each sample is run once: the model iterates until the most conservative
(lowest) threshold triggers termination; all thresholds are checked at every
batch boundary so we get results for all values in a single pass.
...
# ── Multi-threshold TC evaluation (single model pass per sample) ──────────────

def tc_correct_multi(net, inp_np, sol_np, thresholds, device):
    """Run one sample; check every threshold at each batch boundary."""
    ...
    results  = {thr: None for thr in thresholds}   # None = pending
    interims = None
    first    = True
    ...
    # Any still-pending thresholds (shouldn't happen) → failure
...
def main():
    ...
    for ds_name, (inputs, solutions) in datasets.items():
        ...
        # Accumulators: {threshold → correct count}
