...
def main(cfg: DictConfig):
    ...
    # cfg.problem.model.model = "ff_net_recall_2d"

    cfg.problem.model.model_path = os.path.join(cfg.problem.model.model_path, "model_best.pth")###############################

    if cfg.problem.data_type!="zind" and cfg.problem.data_type!="trial":
        terminals=int(re.search(r'\d+', cfg.problem.data_type).group())
    else:
        terminals=10

    if cfg.problem.parallel_testing:
        cfg.problem.model.model = "dt_net_2d_parallel"
    ...
    ####################################################
    #        Test
    # cfg.problem.model.model="feedforward"
    log.info("==> Starting testing...")
    if "feedforward" in cfg.problem.model.model:
        test_iterations = [cfg.problem.model.test_iterations["high"]]
    ...
    if cfg.quick_test:
        test_acc = dt.test(net, [loaders["test"]], cfg.problem.hyp.test_mode, test_iterations, cfg.problem.name, 
                           device,cfg.problem.numchunks,cfg.problem.overlap,terminals,cfg.problem.parallel_testing,cfg.termination_condition,
                           cfg.time_evaluation,cfg.accuracy_evaluation)
        test_acc = test_acc #[0]
        ...
        test_acc, val_acc, train_acc = dt.test(net,
                                               ...
                                               test_iterations,
                                               cfg.problem.name, device,cfg.problem.numchunks,cfg.problem.overlap,terminals,cfg.problem.parallel_testing,cfg.problem.termination_condition,
                                               cfg.time_evaluation,cfg.accuracy_evaluation)
    ...
    stats = OrderedDict([("epochs", cfg.problem.hyp.epochs),
                         ...
                         ("terminals",terminals),
                         ("parallel_testing",cfg.problem.parallel_testing),
