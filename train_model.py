...
def plot_and_save_test_acc_over_epochs(test_accuracies, test_iterations, save_path):
    ...
    for epoch, epoch_accs in enumerate(test_accuracies):
        # Extract the accuracies for each test iteration in this epoch
        accs = [epoch_accs[iteration] for iteration in test_iterations]
        plt.plot(test_iterations, accs, label=f'Epoch {epoch}')  # epoch+1 if you want to start labeling from 1
    ...
    # Save test_accuracies and test_iterations as .npy files
...
def main(cfg: DictConfig):
    ...
    if cfg.retrain==True:
        cfg.problem.model.model_path = cfg.model_path #"/home/gabriel/dpth_copy2/outputs/training_default/training-homeless-Sok/model_best.pth" ################################ retrain from model
...
#####################################################################################
    ...
    #     print(batch[0].shape)  # Assuming batch[0] is the input tensor (features)
    #     break
    # num_items = len(loaders["val"].dataset)
    # print(num_items)
    # num_items_train = len(loaders["train"].dataset)
...
#########################################################################################
    ...
    for epoch in range(start_epoch, cfg.problem.hyp.epochs):
        ...
        ############################################################################################################################################################
        epoch_test_accs = dt.test(net, [loaders["test"]], cfg.problem.hyp.test_mode, test_iterations, cfg.problem.name, device,cfg.problem.numchunks,cfg.problem.overlap,
                                terminals,cfg.problem.parallel_testing,False, #Termination condition, time evaluation, accuracy evaluation
                                False,False)[0] # change test/train
        test_accuracies.append(epoch_test_accs)
        # np.save(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'test_accuracies.npy'), np.array(test_accuracies))
        # np.save(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'test_iterations.npy'), np.array(test_iterations))
    ...
    ############################################################################################################################################################
        # epoch_test_accs = dt.test(net, [loaders["test"]], cfg.problem.hyp.test_mode, test_iterations, cfg.problem.name, device,cfg.problem.numchunks,cfg.problem.overlap)[0]
        # test_accuracies.append(epoch_test_accs)
        np.save(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'test_accuracies.npy'), np.array(test_accuracies))
        np.save(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'test_iterations.npy'), np.array(test_iterations))
    ############################################################################################################################################################
        ...
        if (epoch + 1) % cfg.problem.hyp.val_period == 0 or epoch + 1 == cfg.problem.hyp.epochs:
            test_acc, val_acc, train_acc = dt.test(net,
                                                   ...
                                                   cfg.problem.name,
                                                   device,cfg.problem.numchunks,cfg.problem.overlap)
...
#################################################################################################################################################
    plot_and_save_test_acc_over_epochs(test_accuracies, test_iterations, os.path.dirname(os.path.realpath(__file__)))
#################################################################################################################################################
